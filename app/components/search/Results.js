...
var Results = React.createClass({
  ...
  handleClick: function(item) {
    ...
    helpers.postSaved(item.headline.main,item.web_url).then(function() {
      console.log(item.web_url);
    });
  },

  // A helper method for mapping through our carrier's and outputting some HTML
  renderCarriers: function() {
    return this.props.results.docs.map(function(carrier, index) {
      ...
      return (
        <div key={index}>
          <li className="list-group-item">
            <h3>
              <span>
                <em>{carrier.headline.main}</em>
              </span>
              <span className="btn-group pull-right">
                <a href={carrier.web_url} rel="noopener noreferrer" target="_blank">
                ...
                */}
                <button className="btn btn-primary" onClick={() => this.handleClick(carrier)}>Save</button>
              </span>
            </h3>
            <p>Date Published: {carrier.pub_date}</p>
